...
async function trySilentRefresh({ refreshRaw, res, secret, algorithms, cookieName }) {
  ...
  try {
    ...
    if (!upstream.ok) {
      console.warn('[studio/auth-shim] silent refresh upstream non-ok:', upstream.status);
      return null;
    }
    ...
    const newAccess = setCookies
      ...
      .find((v) => v && v.length > 0);
    if (!newAccess) {
      console.warn('[studio/auth-shim] silent refresh: no new access cookie in upstream Set-Cookie. headers=', setCookies.length);
      return null;
    }

    const { payload } = await jwtVerify(newAccess, secret, { algorithms });
    console.info('[studio/auth-shim] silent refresh succeeded for', payload.email || payload.sub);
    return payload;
  } catch (err) {
    console.warn('[studio/auth-shim] silent refresh threw:', err && err.message ? err.message : String(err));
    return null;
  } finally {
