...
// exposes them by name for use inside TSX components plus three small
// helpers (GIcon, GMonogram, gToneFor) that the canvas uses everywhere.
//
// Re-extraction rule: when Goa canon updates `components/tokens.jsx` in
// the canvas, regenerate this file from canon. See
// `design-systems/goa/DESIGN.md` §15.

import type { CSSProperties } from 'react';

export const GoaTokens = {
  font: 'var(--goa-font)',
...
// Lucide icon — uses the lucide-static iconfont CSS imported in globals.css.
export function GIcon({ name, size = 18, style, className = '' }: GIconProps) {
  return (
    <i
...
// Monogram avatar — used wherever a person/agent has no real image yet.
export function GMonogram({ label = '', tone = 'blue', size = 36, radius }: GMonogramProps) {
  const initials =
    (label || '').split(/\s+/).slice(0, 2).map((s) => s[0] || '').join('').toUpperCase() || '·';

  const tones: Record<GoaTone, { bg: string; fg: string }> = {
...
export function gToneFor(seed = ''): GoaTone {
  let h = 0;
  for (let i = 0; i < seed.length; i++) h = (h * 31 + seed.charCodeAt(i)) | 0;
  return TONES[Math.abs(h) % TONES.length] ?? 'blue';
}
