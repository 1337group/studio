...
// Apply via `applyGoaWallpaper(key, isDark)` or replace with a user image
// via `applyGoaWallpaperImage(url)`.
//
// Default-per-surface convention (from Goa canon):
//   * App-like surfaces (springboard, messages, library, notes, files,
//     studio): `coast`
//   * Settings-like / utility chrome: `graphite` (cyan reads too playful
//     for utility chrome)
//
// Re-extraction rule: when Goa canon updates `components/wallpapers.jsx`
// in the canvas, regenerate this file from canon. See
// `design-systems/goa/DESIGN.md` §15.

export type GoaWallpaperKey =
  | 'coast' | 'dune' | 'forest' | 'rose' | 'graphite' | 'aurora';
...
export const GoaWallpapers: Record<GoaWallpaperKey, WallpaperVariant> = {
  coast: {
    name: 'Coast',
...
export function applyGoaWallpaper(key: GoaWallpaperKey, isDark: boolean): void {
  const wp = GoaWallpapers[key];
  if (!wp) return;
  document.documentElement.style.setProperty('--goa-wallpaper', isDark ? wp.dark : wp.light);
}

export function applyGoaWallpaperImage(url: string): void {
