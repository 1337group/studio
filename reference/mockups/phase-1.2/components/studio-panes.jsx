/* === Studio panes ===========================================================
   Surface 9 — claude.ai/design-style chat+canvas. 5 artboards:
   ...
   Goa warm/quiet style: graphite chrome, gold accents, knot avatar in chat.
============================================================================ */

const STD_GOLD = window.BC_GOLD;

// ─── Shared chrome ─────────────────────────────────────────────────────────
function StdShell({ leftHeader, leftBody, leftFooter, rightHeader, rightBody, leftWidth = 380 }) {
  return (
    <div style={{
      ...
      <div style={{
        ...
        borderRight: '1px solid var(--goa-line)', background: 'var(--goa-bg-elev)',
      }}>
        {leftHeader}
...
function StdChatHeader({ title, subtitle, badge }) {
  return (
    <div style={{
      ...
      <div style={{
          ...
          return (
            <div key={i} style={{ minHeight: line === '' ? 6 : undefined }}>
              {parts.map((p, j) => {
                ...
                if (p.startsWith('`'))  return <code key={j} style={{ fontFamily: 'ui-monospace, SFMono-Regular, monospace', fontSize: 11.5, padding: '1px 5px', borderRadius: 4, background: isMaria ? 'oklch(30% 0.012 250)' : 'var(--goa-bg)', border: isMaria ? 'none' : '1px solid var(--goa-line)' }}>{p.slice(1, -1)}</code>;
        ...
        {m.tool_use && (
          <div style={{
            ...
            background: 'oklch(96% 0.04 80)', border: '1px solid oklch(85% 0.08 80)',
            display: 'flex', alignItems: 'center', gap: 8,
            fontSize: 11.5,
          }}>
            <GIcon name="pencil" size={11} style={{ color: STD_GOLD }} />
            <span style={{ fontFamily: 'ui-monospace, SFMono-Regular, monospace', color: STD_GOLD, fontWeight: 700 }}>{m.tool_use.file}</span>
            <span style={{ color: 'var(--goa-fg-3)' }}>{m.tool_use.summary}</span>
          </div>
        )}
        <div style={{ fontSize: 10.5, marginTop: 4, color: isMaria ? 'oklch(85% 0.005 250 / 0.7)' : 'var(--goa-fg-3)' }}>{m.when}</div>
      </div>
    </div>
  );
}

function StdComposer({ placeholder = 'Ask Goa…' }) {
  return (
    <div style={{ borderTop: '1px solid var(--goa-line)', padding: 14, background: 'var(--goa-bg-elev)' }}>
      <div style={{
        display: 'flex', alignItems: 'flex-end', gap: 6,
        padding: '9px 9px 9px 14px',
        background: 'var(--goa-bg)', border: '1px solid var(--goa-line)', borderRadius: 16,
      }}>
        ...
        }}><GIcon name="arrow-up" size={13} /></button>
      </div>
...
function StdCanvasHeader({ title, subtitle, tabs, actions }) {
  return (
    <div style={{
      display: 'flex', alignItems: 'center', gap: 14, padding: '12px 22px',
      borderBottom: '1px solid var(--goa-line)', background: 'var(--goa-bg-elev)',
    }}>
...
function StdBtn({ icon, label, tone = 'ghost' }) {
  const palettes = {
...
// ─── Live preview canvas (renewal letter) ──────────────────────────────────
function StdLetterPreview({ p = STD_PREVIEW_LETTER }) {
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
        ...
        <span style={{
          ...
          background: 'oklch(96% 0.04 80)', border: '1px solid oklch(85% 0.08 80)',
          fontSize: 11, fontWeight: 700, color: STD_GOLD,
      ...
      <div style={{
        background: '#fff', border: '1px solid var(--goa-line)',
        borderRadius: 12, padding: '36px 44px',
        boxShadow: '0 6px 18px rgba(8, 12, 28, 0.06)',
        maxWidth: 720, margin: '0 auto',
      }}>
        <div style={{
          paddingBottom: 14, borderBottom: '1px solid var(--goa-line)',
          marginBottom: 18,
          ...
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.05em', color: STD_GOLD, textTransform: 'uppercase' }}>Drewlo · Casa Linda</div>
            <div style={{ fontSize: 10.5, color: 'var(--goa-fg-3)' }}>Renewal of lease · 22 abril 2026</div>
          </div>
          <span style={{ flex: 1 }} />
          <span style={{ fontSize: 10.5, color: 'var(--goa-fg-3)', fontFamily: 'ui-monospace, SFMono-Regular, monospace' }}>Apt 4B · {p.current_rent} → {p.new_rent}</span>
        </div>
          ...
          {p.body_es.map((line, i) => (
            <p key={i} style={{ margin: line === '' ? '12px 0' : '0 0 8px', minHeight: line === '' ? 8 : undefined }}>
              {line.split(/(\*\*[^*]+\*\*)/g).map((part, j) =>
                part.startsWith('**') ? <b key={j} style={{ color: STD_GOLD }}>{part.slice(2, -2)}</b> : <span key={j}>{part}</span>
              )}
            </p>
          ))}
        </div>
      ...
      <div style={{
        ...
        background: 'var(--goa-bg-elev)', border: '1px solid var(--goa-line)',
        fontSize: 11.5, color: 'var(--goa-fg-3)',
      }}>
        <GIcon name="clock" size={12} />
        <span>Live preview · re-renders when you edit · last update <b style={{ color: 'var(--goa-fg-2)' }}>2s ago</b></span>
      </div>
    </div>
  );
}

// ─── Pane: App detail (browse, read-only) ──────────────────────────────────
function StdAppDetail() {
  return (
    <StdShell
      leftWidth={400}
      leftHeader={<StdChatHeader title="Studio · Renewals desk" subtitle="Browsing · read-only" badge={
        <span style={{ fontSize: 10.5, fontWeight: 700, padding: '3px 9px', borderRadius: 999, background: 'oklch(95% 0.04 145)', color: 'oklch(40% 0.14 145)', border: '1px solid oklch(85% 0.08 145)', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Production</span>
      } />}
      leftBody={<div style={{ padding: '14px 18px' }}>
        {STD_CHAT_BROWSE.map((m, i) => <StdChatBubble key={i} m={m} />)}
      </div>}
      leftFooter={<div style={{ borderTop: '1px solid var(--goa-line)', padding: 14, background: 'var(--goa-bg-elev)', display: 'flex', gap: 8 }}>
        <StdBtn icon="git-branch" label="Fork & propose changes" tone="primary" />
        <StdBtn icon="file-text" label="See prompt" tone="ghost" />
      </div>}
      rightHeader={<StdCanvasHeader
        title="How it runs"
...
function StdAppDetailCanvas() {
  return (
    <div style={{ height: '100%', overflowY: 'auto', padding: '24px 32px', background: 'oklch(96% 0.005 240)' }}>
      <div style={{
        background: '#fff', border: '1px solid var(--goa-line)', borderRadius: 14,
        padding: '22px 26px', maxWidth: 760, margin: '0 auto',
      }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, marginBottom: 18 }}>
          <div style={{
            width: 48, height: 48, borderRadius: 12,
            background: STD_GOLD, color: '#fff',
            display: 'inline-flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
          }}><GIcon name="file-text" size={22} /></div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <h2 style={{ fontSize: 22, fontWeight: 700, letterSpacing: '-0.02em', margin: 0, color: 'var(--goa-fg)' }}>{STD_FOCUS_APP.title}</h2>
            <div style={{ fontSize: 12, color: 'var(--goa-fg-3)', marginTop: 3 }}>by {STD_FOCUS_APP.owner} · {STD_FOCUS_APP.used_by} active users · {STD_FOCUS_APP.forks} forks</div>
            <div style={{ fontSize: 13.5, color: 'var(--goa-fg-2)', lineHeight: 1.55, marginTop: 8 }}>{STD_FOCUS_APP.blurb}</div>
          </div>
        </div>

        <div style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: '0.04em', textTransform: 'uppercase', color: 'var(--goa-fg-3)', marginBottom: 8 }}>How it works</div>
        {[
          ...
          <div key={s.n} style={{
            ...
            gap: 12, padding: '10px 0', borderBottom: '1px solid var(--goa-line)',
          }}>
            <div style={{
              width: 26, height: 26, borderRadius: 13,
              background: 'oklch(96% 0.04 80)', color: STD_GOLD,
        ...
        <div style={{
          ...
          fontSize: 12, color: 'var(--goa-fg-2)', lineHeight: 1.5,
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4, fontWeight: 700, color: 'var(--goa-fg)' }}>
            <GIcon name="info" size={12} style={{ color: STD_GOLD }} /> What you can change
          </div>
          When you fork, you can edit the prompt, change the rate-card numbers, add tools, or branch the logic by tenant type. You can't change the data sources — those stay read-only.
        </div>
      </div>
    </div>
  );
...
// ─── Pane: Propose changes (HERO) ──────────────────────────────────────────
function StdPropose() {
  const [tab, setTab] = React.useState('preview');
  return (
    <StdShell
      leftWidth={420}
      leftHeader={<StdChatHeader title="Studio · Renewals desk" subtitle="Your fork · 1 file modified · saved 14s ago" badge={
        <span style={{ fontSize: 10.5, fontWeight: 700, padding: '3px 9px', borderRadius: 999, background: 'oklch(96% 0.04 80)', color: STD_GOLD, border: '1px solid oklch(85% 0.08 80)', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Draft</span>
      } />}
      leftBody={<div style={{ padding: '14px 18px' }}>
        {STD_CHAT_PROPOSE.map((m, i) => <StdChatBubble key={i} m={m} />)}
      </div>}
      leftFooter={<>
      ...
      rightHeader={<StdCanvasHeader
        ...
        tabs={
          <div style={{ display: 'inline-flex', background: 'var(--goa-surface-2)', borderRadius: 8, border: '1px solid var(--goa-line)', marginLeft: 10 }}>
            {[{id:'preview', label:'Preview', icon: 'eye'}, {id:'diff', label:'Diff', icon: 'git-compare'}, {id:'tests', label:'Tests', icon: 'check-circle'}].map(t => (
              <button key={t.id} onClick={() => setTab(t.id)} style={{
                ...
                margin: 2,
              }}><GIcon name={t.icon} size={11} />{t.label}</button>
            ))}
          </div>
        }
        actions={<>
      ...
      />}
      rightBody={
        tab === 'preview' ? <StdLetterPreview /> :
        tab === 'diff'    ? <StdDiffView /> :
                            <StdTestsView />
      }
    />
  );
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', padding: '20px 28px', background: 'var(--goa-bg)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
        <span style={{ fontFamily: 'ui-monospace, SFMono-Regular, monospace', fontSize: 12.5, fontWeight: 700, color: 'var(--goa-fg)' }}>system_prompt.md</span>
        <span style={{ fontSize: 11, color: 'var(--goa-fg-3)' }}>4 added · 1 removed</span>
      </div>
      <div style={{
        background: 'var(--goa-bg-elev)', border: '1px solid var(--goa-line)', borderRadius: 10,
        fontFamily: 'ui-monospace, SFMono-Regular, monospace', fontSize: 12.5, lineHeight: 1.65,
        overflow: 'hidden',
      }}>
        {STD_DIFF.map((row, i) => {
          const bg = row.kind === 'add'    ? 'oklch(96% 0.05 145)' :
                     row.kind === 'remove' ? 'oklch(96% 0.05 25)'  : 'transparent';
          const fg = row.kind === 'add'    ? 'oklch(35% 0.12 145)' :
                     row.kind === 'remove' ? 'oklch(40% 0.14 25)'  : 'var(--goa-fg)';
          const sign = row.kind === 'add' ? '+' : row.kind === 'remove' ? '−' : ' ';
          return (
            <div key={i} style={{
              display: 'grid', gridTemplateColumns: '52px 24px 1fr',
              background: bg, color: fg,
              padding: '0 14px',
            }}>
              <span style={{ textAlign: 'right', paddingRight: 12, color: 'var(--goa-fg-3)', userSelect: 'none' }}>{row.line}</span>
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', padding: '24px 32px', background: 'var(--goa-bg)' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {STD_TESTS.map(t => {
          const tone = t.status === 'pass' ? { bg: 'oklch(96% 0.04 145)', fg: 'oklch(40% 0.14 145)', border: 'oklch(85% 0.08 145)', icon: 'check-circle' }
                     : t.status === 'warn' ? { bg: 'oklch(96% 0.05 75)',  fg: 'oklch(45% 0.14 75)',  border: 'oklch(85% 0.10 75)',  icon: 'alert-circle' }
          ...
          return (
            <div key={t.id} style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontFamily: 'ui-monospace, SFMono-Regular, monospace', fontSize: 12.5, fontWeight: 700, color: tone.fg }}>{t.id}</div>
...
// ─── Pane: Build from scratch ──────────────────────────────────────────────
function StdScratch() {
  return (
    <StdShell
      leftWidth={420}
      leftHeader={<StdChatHeader title="Studio · New project" subtitle="Your own Docker workspace · Hive backend ready" />}
      leftBody={<div style={{ padding: '14px 18px' }}>
        {STD_CHAT_SCRATCH.map((m, i) => <StdChatBubble key={i} m={m} />)}
      </div>}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ fontSize: 17, fontWeight: 700, letterSpacing: '-0.015em', color: 'var(--goa-fg)' }}>Tell Goa what you need</div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 7, marginTop: 14, textAlign: 'left' }}>
          {[
            ...
            <button key={s} style={{
              ...
              background: 'var(--goa-surface-2)', border: '1px solid var(--goa-line)',
              fontSize: 12.5, color: 'var(--goa-fg-2)',
              display: 'flex', alignItems: 'center', gap: 8,
            }}>
...
// ─── Pane: Launch (Dev → Beta → Prod stepper) ──────────────────────────────
function StdLaunch() {
  return (
    <div style={{ width: '100%', height: '100%', overflowY: 'auto', background: 'oklch(96% 0.005 240)', padding: '32px 40px' }}>
      <div style={{ maxWidth: 920, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, marginBottom: 22 }}>
          <div style={{
            ...
            background: STD_GOLD, color: '#fff',
            display: 'inline-flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
          }}><GIcon name="rocket" size={24} /></div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.05em', color: STD_GOLD, textTransform: 'uppercase' }}>Launch</div>
            <h2 style={{ fontSize: 26, fontWeight: 700, letterSpacing: '-0.02em', margin: '2px 0 4px', color: 'var(--goa-fg)' }}>{STD_LAUNCH_APP.name}</h2>
            <div style={{ fontSize: 13, color: 'var(--goa-fg-3)' }}>by {STD_LAUNCH_APP.author} · built in {STD_LAUNCH_APP.built_in} · {STD_LAUNCH_APP.iterations} iterations</div>
          </div>
            ...
            return (
              <div key={s.id} style={{ position: 'relative' }}>
                {i < STD_LAUNCH_STEPS.length - 1 && (
                  <div style={{
                    position: 'absolute', left: 'calc(100% + 7px)', top: 16,
                    width: 14, height: 1, background: 'var(--goa-line)',
                  }} />
                )}
                <div style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--goa-fg)', letterSpacing: '-0.01em' }}>{s.label}</div>
                    <div style={{ fontSize: 10.5, fontWeight: 600, color: isCurrent ? STD_GOLD : 'var(--goa-fg-3)', textTransform: 'uppercase', letterSpacing: '0.04em' }}>{s.hint}</div>
                  </div>
                </div>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 14 }}>
            <div>
              ...
              <div style={{ fontSize: 11, color: 'var(--goa-fg-3)' }}>3 of 10 selected · they\u2019ll see this app on their springboard immediately</div>
            </div>
            ...
            {STD_BETA_TESTERS.map(t => (
              <div key={t.name} style={{
                ...
                <span style={{
                  ...
                  color: '#fff',
                  display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
                }}>{t.selected && <GIcon name="check" size={10} />}</span>
                <div style={{
          ...
          <div style={{ borderRadius: 9, overflow: 'hidden', border: '1px solid var(--goa-line)' }}>
            {STD_HIVE_ENV.map((e, i) => (
              <div key={e.key} style={{
                ...
                <span style={{ fontFamily: 'ui-monospace, SFMono-Regular, monospace', fontSize: 12, fontWeight: 700, color: STD_GOLD }}>{e.key}</span>
                <span style={{ fontFamily: 'ui-monospace, SFMono-Regular, monospace', fontSize: 12, color: 'var(--goa-fg)' }}>{e.value}</span>
...
// ─── Pane: Review queue (Allan's view) ─────────────────────────────────────
function StdReview() {
  ...
  return (
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', background: 'var(--goa-bg)' }}>
      <div style={{
        ...
        <div>
          <div style={{ fontSize: 17, fontWeight: 700, letterSpacing: '-0.015em', color: 'var(--goa-fg)' }}>Review queue</div>
          <div style={{ fontSize: 11.5, color: 'var(--goa-fg-3)' }}>3 awaiting review · 1 changes requested · 1 approved this week</div>
        </div>
        <div style={{ flex: 1 }} />
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '380px 1fr', minHeight: 0 }}>
        {/* List */}
        <div style={{ borderRight: '1px solid var(--goa-line)', overflowY: 'auto', background: 'var(--goa-bg-elev)' }}>
            ...
            <div key={p.id} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                <div style={{
                  ...
                  background: p.avatar_tone, color: '#fff',
                  display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: 9.5, fontWeight: 700,
                }}>{p.initials}</div>
                <span style={{ fontSize: 11, fontFamily: 'ui-monospace, SFMono-Regular, monospace', color: 'var(--goa-fg-3)' }}>{p.id}</span>
              ...
              <div style={{ fontSize: 13.5, fontWeight: p.selected ? 700 : 600, color: 'var(--goa-fg)', lineHeight: 1.35 }}>{p.title}</div>
              <div style={{ fontSize: 11.5, color: 'var(--goa-fg-3)', marginTop: 2 }}>{p.author} · {p.when}</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 6, fontSize: 10.5, color: 'var(--goa-fg-3)', fontFamily: 'ui-monospace, SFMono-Regular, monospace' }}>
                ...
                <span style={{ flex: 1 }} />
                <span><b style={{ color: 'oklch(40% 0.14 145)' }}>{p.tests.passing}</b> ✓</span>
                {p.tests.warning > 0 && <span><b style={{ color: 'oklch(48% 0.16 75)' }}>{p.tests.warning}</b> !</span>}
                {p.tests.failing > 0 && <span><b style={{ color: 'oklch(45% 0.16 25)' }}>{p.tests.failing}</b> ✗</span>}
              </div>
            </div>
          ))}
        </div>
...
function StdReviewStatus({ status }) {
  const tones = {
    'awaiting-review':   { label: 'Awaiting review',   bg: 'oklch(96% 0.04 80)',  fg: STD_GOLD,            border: 'oklch(85% 0.08 80)' },
    'changes-requested': { label: 'Changes requested', bg: 'oklch(96% 0.05 25)',  fg: 'oklch(48% 0.16 25)', border: 'oklch(85% 0.08 25)' },
  ...
  return (
    <div style={{ overflowY: 'auto', padding: '24px 32px', background: 'var(--goa-bg)' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 16 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 11, color: 'var(--goa-fg-3)', fontFamily: 'ui-monospace, SFMono-Regular, monospace' }}>{p.id} · {p.app}</div>
          <h2 style={{ fontSize: 22, fontWeight: 700, letterSpacing: '-0.02em', margin: '2px 0 4px', color: 'var(--goa-fg)' }}>{p.title}</h2>
          <div style={{ fontSize: 12, color: 'var(--goa-fg-3)' }}>by {p.author} · {p.when}</div>
        </div>
      ...
      <div style={{
        ...
        <StdReviewKpi icon="git-pull-request" label="Diff" value={`+${p.diff_stat.added} / −${p.diff_stat.removed}`} hint={`${p.diff_stat.files} files`} />
        <StdReviewKpi icon="check-circle" label="Tests" value={`${p.tests.passing} pass`} hint={p.tests.warning ? `${p.tests.warning} warning` : 'all green'} tone={p.tests.warning ? 'warn' : 'good'} />
        <StdReviewKpi icon="users" label="Affected users" value="47" hint="active users on this app" />
      </div>

      <div style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: '0.04em', textTransform: 'uppercase', color: 'var(--goa-fg-3)', marginBottom: 8 }}>What changed</div>
      <StdDiffView />

      <div style={{
        position: 'sticky', bottom: 0, marginTop: 18, padding: '14px 16px',
        background: 'var(--goa-bg-elev)', border: '1px solid var(--goa-line)', borderRadius: 12,
        display: 'flex', alignItems: 'center', gap: 10,
...
function StdReviewKpi({ icon, label, value, hint, tone = 'neutral' }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 10.5, fontWeight: 700, letterSpacing: '0.04em', textTransform: 'uppercase', color: 'var(--goa-fg-3)' }}>
        <GIcon name={icon} size={11} /> {label}
      </div>
      <div style={{ fontSize: 17, fontWeight: 700, letterSpacing: '-0.015em', color: fg, marginTop: 3 }}>{value}</div>
...
Object.assign(window, {
  StdAppDetail, StdPropose, StdScratch, StdLaunch, StdReview,
});
